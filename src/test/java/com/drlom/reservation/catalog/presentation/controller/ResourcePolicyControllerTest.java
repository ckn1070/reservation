...
class ResourcePolicyControllerTest {
  ...
  @Nested
  @DisplayName("POST /api/resources/{resourceId}/policies")
  class CreateResourcePolicy {
    ...
    void createResourcePolicy_stringValue_success() throws Exception {
      Map<String, Object> request = Map.of("policyType", "MAX_BOOKING", "valueString", "10");
      ...
      mockMvc.perform(post("/api/resources/1/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.policyType").value("MAX_BOOKING"));
    }
    ...
    void createResourcePolicy_numberValue_success() throws Exception {
      Map<String, Object> request = Map.of("policyType", "DISCOUNT_RATE", "valueNumber", 10.5);
      ...
      given(createResourcePolicyUseCase.execute(any(CreateResourcePolicyCommand.class))).willReturn(result);
      mockMvc.perform(post("/api/resources/1/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andExpect(jsonPath("$.policyType").value("DISCOUNT_RATE"));
    }
    ...
    void createResourcePolicy_forbidden() throws Exception {
      Map<String, Object> request = Map.of("policyType", "MAX_BOOKING", "valueString", "10");
      mockMvc.perform(post("/api/resources/1/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isForbidden());
    }
    ...
    void createResourcePolicy_unauthorized() throws Exception {
      Map<String, Object> request = Map.of("policyType", "MAX_BOOKING", "valueString", "10");
      mockMvc.perform(post("/api/resources/1/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isUnauthorized());
    }
    ...
    void createResourcePolicy_validationError() throws Exception {
      Map<String, Object> request = Map.of("valueString", "10");
      mockMvc.perform(post("/api/resources/1/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isBadRequest());
    }
    ...
    void createResourcePolicy_resourceNotFound() throws Exception {
      ...
      mockMvc.perform(post("/api/resources/999/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isNotFound());
    }
    ...
    void createResourcePolicy_duplicatePolicy() throws Exception {
      ...
      mockMvc.perform(post("/api/resources/1/policies").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isConflict());
    }
  }
}
