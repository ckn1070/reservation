...
class ResourceRateControllerTest {
  ...
  class CreateResourceRate {
    ...
    void createResourceRate_base_success() throws Exception {
      ...
      mockMvc.perform(post("/api/resources/1/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.rateType").value("BASE")).andExpect(jsonPath("$.amount").value(50000));
    }
    ...
    void createResourceRate_promotion_success() throws Exception {
      ...
      Map<String, Object> request = Map.of("rateType", "PROMOTION", "amount", 40000L, "startAt", "2026-02-01T00:00:00", "endAt", "2026-02-28T23:59:00", "priority", 10, "reason", "설날 프로모션");
      ...
      given(createResourceRateUseCase.execute(any(CreateResourceRateCommand.class))).willReturn(result);
      mockMvc.perform(post("/api/resources/1/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andExpect(jsonPath("$.rateType").value("PROMOTION")).andExpect(jsonPath("$.amount").value(40000));
    }
    ...
    void createResourceRate_asSuperAdmin_success() throws Exception {
      ...
      ResourceRateResult result = ResourceRateResult.builder().id(1L).resourceId(1L).rateType(RateType.BASE).amount(50000L).currency("KRW").startAt(null).endAt(null).priority(0).reason(null).build();
      given(createResourceRateUseCase.execute(any(CreateResourceRateCommand.class))).willReturn(result);
      mockMvc.perform(post("/api/resources/1/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.rateType").value("BASE"));
    }
    ...
    void createResourceRate_forbidden() throws Exception {
      Map<String, Object> request = Map.of("rateType", "BASE", "amount", 50000L);
      mockMvc.perform(post("/api/resources/1/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isForbidden());
    }
    ...
    void createResourceRate_unauthorized() throws Exception {
      Map<String, Object> request = Map.of("rateType", "BASE", "amount", 50000L);
      mockMvc.perform(post("/api/resources/1/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isUnauthorized());
    }
    ...
    void createResourceRate_validationError() throws Exception {
      Map<String, Object> request = Map.of("amount", 50000L);
      mockMvc.perform(post("/api/resources/1/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isBadRequest());
    }
    ...
    void createResourceRate_resourceNotFound() throws Exception {
      ...
      mockMvc.perform(post("/api/resources/999/rates").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isNotFound());
    }
    ...
    void createResourceRate_invalidRateType() throws Exception {
      Map<String, Object> request = Map.of("rateType", "INVALID_TYPE", "amount", 50000L);
